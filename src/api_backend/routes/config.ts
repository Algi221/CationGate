...
function saveBase64File(base64Str: string, prefix: string): string {
  ...
  try {
    ...
    // Validasi tipe konten yang diperbolehkan
    const allowedTypes = [
...
configRouter.post('/', adminAuth, async (c) => {
  try {
    ...
    await prisma.landingPageConfig.upsert({
      ...
      update: { config_value: processedValue as any, updated_at: new Date() },
      create: { config_key: key, config_value: processedValue as any }
    });
...
configRouter.get('/revisions', adminAuth, async (c) => {
  try {
    const revisions = await prisma.uIRevision.findMany({
      select: {
        id: true,
        changed_by: true,
        description: true,
...
configRouter.post('/save-all', adminAuth, async (c) => {
  try {
    ...
    for (const [key, value] of Object.entries(processedConfigs)) {
      await prisma.landingPageConfig.upsert({
        where: { config_key: key },
        update: { config_value: value as any, updated_at: new Date() },
        create: { config_key: key, config_value: value as any }
      });
    }

    // Insert a new revision record into history log
    const admin = (c as any).get('admin');
    const adminName = admin?.nama || admin?.username || 'Administrator';
    
    await prisma.uIRevision.create({
      data: {
        config_values: configs as any,
...
configRouter.post('/restore', adminAuth, async (c) => {
  try {
    ...
    // Fetch the target revision from database
    const revision = await prisma.uIRevision.findUnique({
    ...
    const configs = typeof config_values === 'string' ? JSON.parse(config_values) : (config_values as any);
    ...
    // Insert a new revision record documenting the restoration
    const admin = (c as any).get('admin');
    const adminName = admin?.nama || admin?.username || 'Administrator';
