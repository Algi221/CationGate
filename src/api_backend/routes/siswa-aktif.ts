...
siswaAktifRouter.get('/', adminAuth, async (c: Context) => {
  try {
    ...
    const selectFields = siswaAktifFields.reduce((acc: any, field: string) => {
      acc[field] = true;
      return acc;
    }, {});

    const rows = await prisma.siswaAktif.findMany({
      select: selectFields,
      orderBy: {
        nama: 'asc'
      }
...
siswaAktifRouter.get('/:id', adminAuth, async (c: Context) => {
  try {
    ...
    const record = await prisma.siswaAktif.findUnique({
      where: { id }
    });
...
siswaAktifRouter.put('/:id', adminAuth, async (c: Context) => {
  try {
    ...
    const existingRecord = await prisma.siswaAktif.findUnique({
      where: { id }
    });
    ...
    const updatedRecord = await prisma.siswaAktif.update({
      where: { id },
      data: fields
    });
...
siswaAktifRouter.delete('/:id', adminAuth, async (c: Context) => {
  try {
    const id = parseInt(c.req.param('id') || '0');
    const student = await prisma.siswaAktif.findUnique({
      where: { id }
    });
    ...
    // Delete active student record
    await prisma.siswaAktif.delete({
      where: { id }
    });

    // Also update original registration status back to 'Pending' so they aren't marked approved anymore
    if (student.calon_siswa_id) {
      await prisma.calonSiswa.update({
        where: { id: student.calon_siswa_id },
        data: {
          status: 'Pending',
          diterima_kelas: null,
      ...
      // Fetch the updated CalonSiswa to broadcast
      const updatedApplicant = await prisma.calonSiswa.findUnique({
        where: { id: student.calon_siswa_id }
      });
      if (updatedApplicant) {
...
siswaAktifRouter.post('/generate-nipd', adminAuth, async (c: Context) => {
  try {
    ...
    // Default prefix logic: e.g., "2627" for 2026-2027
    ...
    const students = await prisma.siswaAktif.findMany();

    // Grouping by Jurusan based on specific order: TE, RPL, TJKT (TKJ), BC (PSPT), Animasi (ANM)
    ...
    // Execute updates sequentially or in transaction
    await prisma.$transaction(updates);

    // Broadcast full refresh
    broadcast({ event: 'STUDENT_NIPD_GENERATED', data: { count: updates.length } }, true);

    return c.json({
      success: true,
      message: `Berhasil men-generate NIPD untuk ${updates.length} siswa secara berurutan.`
    });
  } catch (err: any) {
...
siswaAktifRouter.post('/:id/mutasi', adminAuth, async (c: Context) => {
  try {
    ...
    const transaction = await prisma.$transaction(async (tx) => {
      // 1. Update SiswaAktif
      const updatedSiswa = await tx.siswaAktif.update({
        where: { id },
        data: {
          jurusan: jurusan_baru,
          diterima_kelas: diterima_kelas_baru || student.diterima_kelas,
          nipd: null // Reset NIPD so it can be regenerated with correct sorting
        }
      });

      // 2. Insert MutasiHistory
      await tx.mutasiHistory.create({
        data: {
          siswa_aktif_id: id,
          jurusan_asal: jurusanAsal,
          jurusan_tujuan: jurusan_baru,
          dilakukan_oleh: (c.get('admin') as any)?.nama_lengkap || (c.get('admin') as any)?.username || 'Admin'
        }
      });

      // 3. Update CalonSiswa if linked
      if (updatedSiswa.calon_siswa_id) {
        await tx.calonSiswa.update({
          where: { id: updatedSiswa.calon_siswa_id },
          data: {
    ...
    broadcast({ event: 'STUDENT_UPDATED', data: transaction }, true);
