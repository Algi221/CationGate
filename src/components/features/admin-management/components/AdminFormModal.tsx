...
import { Save, User, KeyRound, Eye, EyeOff, Mail, ShieldCheck, Sparkles } from "lucide-react";
    ...
    >
      <div className="flex items-center justify-between mb-4 border-b border-slate-100 dark:border-slate-800 pb-3">
        <div>
          <h2 className="text-base font-extrabold text-slate-800 dark:text-white flex items-center gap-2">
            ...
            {editAdminId ? "Edit Akun Admin" : "Tambah Akun Admin Baru"}
          </h2>
          <p className="text-xs text-slate-400 font-medium mt-0.5">
            {editAdminId
              ? "Perbarui kredensial dan informasi akun admin sekolah."
              : "Buat akun admin sekolah dengan email dan kata sandi yang valid."}
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <div className="relative flex items-center">
              <ShieldCheck size={16} className="absolute left-3.5 text-emerald-500" />
              <input
                type="text"
                disabled
                readOnly
                value="Admin"
                className="w-full bg-slate-100 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 rounded-2xl pl-10 pr-4 py-3 text-xs font-black text-slate-800 dark:text-white cursor-not-allowed opacity-90"
              />
            </div>
          </div>
          ...
          <div className="md:col-span-2">
            <label className="block text-xs font-black uppercase tracking-wider text-slate-500 mb-1.5">
              {editAdminId ? "Kata Sandi Baru (Kosongkan jika tidak diubah)" : "Kata Sandi Admin"} {!editAdminId && <span className="text-rose-500">*</span>}
            </label>
            <div className="relative">
              ...
              <input
                ...
                placeholder={editAdminId ? "•••••••• (Kosongkan jika tidak diubah)" : "Masukkan kata sandi login admin (min. 6 karakter)"}
        ...
        <div className="flex justify-end gap-3 pt-2">
          ...
          >
            <Save size={15} />
            {formLoading ? "Menyimpan..." : editAdminId ? "Simpan Perubahan" : "Buat Akun Admin"}
          </button>
        </div>
      </form>
    </motion.div>
