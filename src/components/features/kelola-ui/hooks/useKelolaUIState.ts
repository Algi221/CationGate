...
import {
  parseConfigArray,
  sanitizeForDraft,
  buildKelolaUISnapshot,
} from "../utils/kelolaUIHelpers";
import { useKelolaUIMedia } from "./useKelolaUIMedia";
import { useKelolaUIRevisions } from "./useKelolaUIRevisions";
...
export function useKelolaUIState() {
  ...
  const schoolSlug =
    ...
    !window.location.hostname.startsWith("gatekeeper.")
  ...
  const [heroTitle, setHeroTitle] = useState(() =>
    isDemo ? "Penerimaan Siswa Baru" : "",
  );
  const [heroTitleSub, setHeroTitleSub] = useState(() =>
    isDemo ? "Portal PPDB" : "",
  );
  const [heroSubtitle, setHeroSubtitle] = useState(() =>
    isDemo ? "Platform pendaftaran peserta didik baru resmi." : "",
  );
  const [heroBgImage, setHeroBgImage] = useState<string>("");
  const [phone, setPhone] = useState(() => (isDemo ? "+62218740756" : ""));
  const [email, setEmail] = useState(() =>
    isDemo ? "info@smktarunabhakti.sch.id" : "",
  );
  const [address, setAddress] = useState(() =>
    isDemo
      ? "Jl. Pekapuran RT 02 RW 06, Curug, Cimanggis, Kota Depok, Jawa Barat 16453"
      : "",
  );
  const [footerDesc, setFooterDesc] = useState(() =>
    isDemo
      ? "Pionir pendidikan kejuruan teknologi informasi di Kota Depok dengan sertifikasi internasional dan industri."
      : "",
  );
  const [mapTitle, setMapTitle] = useState(() =>
    isDemo ? "Kunjungi Kampus SMK Taruna Bhakti" : "",
  );
  ...
  const [waGroupUrl, setWaGroupUrl] = useState(() =>
    isDemo ? "https://chat.whatsapp.com/HJXHYajEOhl5RM6iN2SJOS" : "",
  );
  const [waAdmin, setWaAdmin] = useState(() =>
    isDemo ? "6281292244456" : "",
  );
  const [formGuideline, setFormGuideline] = useState(() =>
    isDemo
      ? "Silakan isi formulir pendaftaran calon siswa dengan lengkap dan benar. Berkas persyaratan wajib diunggah dalam format gambar (PNG/JPG) maksimal 2MB."
      : "",
  );
  ...
  const [alurList, setAlurList] = useState<AlurItem[]>(() => {
    ...
    if (typeof window !== "undefined") {
      const saved =
        (slug ? localStorage.getItem(`ppdb_alur_config_${slug}`) : null) ||
        localStorage.getItem("ppdb_alur_config");
      if (saved) {
        try {
  ...
  const [majorsList, setMajorsList] = useState<MajorItem[]>(() => {
    ...
    if (typeof window !== "undefined") {
      const saved =
        (slug ? localStorage.getItem(`ppdb_majors_config_${slug}`) : null) ||
  ...
  const [faqList, setFaqList] = useState<FaqItem[]>(() => {
    ...
    if (typeof window !== "undefined") {
      const saved =
        (slug ? localStorage.getItem(`ppdb_faq_config_${slug}`) : null) ||
        localStorage.getItem("ppdb_faq_config");
      if (saved) {
        try {
          const parsed = JSON.parse(saved);
          if (Array.isArray(parsed) && parsed.length > 0) return parsed;
  ...
  const [faqTitle, setFaqTitle] = useState(() =>
    isDemo ? "Pertanyaan yang Sering Diajukan" : "",
  );
  const [faqSubtitle, setFaqSubtitle] = useState(() =>
    isDemo
      ? "Temukan jawaban cepat untuk kendala dan pertanyaan seputar proses pendaftaran."
      : "",
  );
  ...
  const [partnersList, setPartnersList] = useState<PartnerItem[]>(() => {
    ...
    if (typeof window !== "undefined") {
      const saved =
        (slug ? localStorage.getItem(`ppdb_partners_config_${slug}`) : null) ||
        localStorage.getItem("ppdb_partners_config");
      if (saved) {
        try {
          const parsed = JSON.parse(saved);
          if (Array.isArray(parsed) && parsed.length > 0) return parsed;
  ...
  const [bankConfigList, setBankConfigList] = useState<BankConfigItem[]>(() => {
    if (isDemo)
      return [
        {
          bankName: "Bank BJB",
          accountNumber: "0010203040506",
          accountHolder: "SMK Taruna Bhakti",
        },
      ];
    if (typeof window !== "undefined") {
      const saved =
        (slug ? localStorage.getItem(`ppdb_bank_config_${slug}`) : null) ||
        localStorage.getItem("ppdb_bank_config");
      if (saved) {
        try {
          const parsed = JSON.parse(saved);
          if (Array.isArray(parsed) && parsed.length > 0) return parsed;
  ...
  const [fieldsConfigUI, setFieldsConfigUI] = useState<
    Record<string, FieldConfigItem>
  >(() => {
    if (typeof window !== "undefined") {
      const saved =
        (slug ? localStorage.getItem(`ppdb_fields_config_${slug}`) : null) ||
        localStorage.getItem("ppdb_fields_config");
      if (saved) {
        try {
          const parsed = JSON.parse(saved);
          if (parsed && typeof parsed === "object")
            return { ...DEFAULT_FIELDS_CONFIG_UI, ...parsed };
        } catch (_) {}
      }
    }
  ...
  const [gelombangConfig, setGelombangConfig] = useState<{
    ...
    if (typeof window !== "undefined") {
      const saved =
        (slug
          ? localStorage.getItem(`ppdb_gelombang_config_${slug}`)
          : null) || localStorage.getItem("ppdb_gelombang_config");
      if (saved) {
        try {
          const parsed = JSON.parse(saved);
  ...
  // Sub-hook: Media Upload & Compression
  const {
    dragActiveStates,
    setDragActiveStates,
    handleDragState,
  ...
  const isDirty = useMemo(() => {
    ...
    const current = buildKelolaUISnapshot({
  ...
  const fetchCurrentConfig = useCallback(async () => {
    try {
      ...
      // Server Database always takes precedence over stale drafts
      const hasServerConfig = Object.keys(config).length > 0;
      const activeConfig = hasServerConfig ? { ...config } : draft || {};

      if (activeConfig.ppdb_hero_title)
        setHeroTitle(activeConfig.ppdb_hero_title);
      if (activeConfig.ppdb_hero_title_sub)
        setHeroTitleSub(activeConfig.ppdb_hero_title_sub);
      else if (!draft)
        setHeroTitleSub(`Portal PPDB ${ppdbTitle || "Online"}`);

      if (activeConfig.ppdb_hero_subtitle)
        setHeroSubtitle(activeConfig.ppdb_hero_subtitle);
      if (activeConfig.ppdb_hero_bg_image)
        setHeroBgImage(activeConfig.ppdb_hero_bg_image);
      if (activeConfig.ppdb_phone)
        setPhone(formatPhoneNumber(activeConfig.ppdb_phone));
      if (activeConfig.ppdb_email) setEmail(activeConfig.ppdb_email);
      if (activeConfig.ppdb_address || activeConfig.ppdb_alamat)
        setAddress(activeConfig.ppdb_address || activeConfig.ppdb_alamat);
      if (activeConfig.ppdb_map_title)
        setMapTitle(activeConfig.ppdb_map_title);
      if (activeConfig.ppdb_map_url || activeConfig.ppdb_maps_embed)
        setMapUrl(activeConfig.ppdb_map_url || activeConfig.ppdb_maps_embed);
      if (activeConfig.ppdb_school_period)
        setSchoolPeriod(activeConfig.ppdb_school_period);
      if (activeConfig.ppdb_faq_title)
        setFaqTitle(activeConfig.ppdb_faq_title);
      if (activeConfig.ppdb_faq_subtitle)
        setFaqSubtitle(activeConfig.ppdb_faq_subtitle);
      if (activeConfig.ppdb_wa_group_url)
        setWaGroupUrl(activeConfig.ppdb_wa_group_url);
      if (activeConfig.ppdb_wa_admin)
        setWaAdmin(formatPhoneNumber(activeConfig.ppdb_wa_admin));
      if (activeConfig.ppdb_form_guideline)
        setFormGuideline(activeConfig.ppdb_form_guideline);
      if (activeConfig.ppdb_form_fee)
        setFormFee(activeConfig.ppdb_form_fee);
      if (activeConfig.ppdb_logo_url)
        setSchoolLogo(activeConfig.ppdb_logo_url);

      if (activeConfig.ppdb_title)
        setSchoolTitle(activeConfig.ppdb_title);
      if (activeConfig.ppdb_footer_desc)
        setFooterDesc(activeConfig.ppdb_footer_desc);
      else if (!draft && isDemo)
        setSchoolTitle(`PPDB ${ppdbTitle || "Sekolah"}`);

      const parsedAlur = parseConfigArray<AlurItem>(
        activeConfig.ppdb_alur_config,
      );
        ...
        if (typeof window !== "undefined") {
          if (slug)
            localStorage.setItem(
              `ppdb_alur_config_${slug}`,
              JSON.stringify(parsedAlur),
            );
          localStorage.setItem(
            `ppdb_alur_config`,
            JSON.stringify(parsedAlur),
      ...
      const parsedFaq = parseConfigArray<FaqItem>(
        activeConfig.ppdb_faq_config,
      );
      if (parsedFaq && parsedFaq.length > 0) {
        ...
        if (typeof window !== "undefined") {
          if (slug)
            localStorage.setItem(
              `ppdb_faq_config_${slug}`,
              JSON.stringify(parsedFaq),
            );
      ...
      const parsedPartners = parseConfigArray<PartnerItem>(
        activeConfig.ppdb_partners_config,
      );
      if (parsedPartners && parsedPartners.length > 0) {
        ...
        if (typeof window !== "undefined") {
          if (slug)
            localStorage.setItem(
              `ppdb_partners_config_${slug}`,
              JSON.stringify(parsedPartners),
            );
          localStorage.setItem(
        ...
        const cachedPartners =
          (slug
            ? localStorage.getItem(`ppdb_partners_config_${slug}`)
            : null) || localStorage.getItem("ppdb_partners_config");
        if (cachedPartners) {
          try {
            const parsed = JSON.parse(cachedPartners);
            if (Array.isArray(parsed) && parsed.length > 0)
              setPartnersList(parsed);
          } catch (_) {}
        }
      }
      ...
      if (isDemo) {
        ...
        const parsedMajors = parseConfigArray<MajorItem>(
          activeConfig.ppdb_majors_config,
        );
        if (parsedMajors && parsedMajors.length > 0) {
          ...
          const mergedMajors = parsedMajors.map((dbMajor: any) => {
            ...
            return {
              ...
              careers: Array.isArray(dbMajor.careers)
                ? dbMajor.careers
                : typeof dbMajor.careers === "string"
                ? dbMajor.careers.split(",").map((s: string) => s.trim())
                : [],
              facilities: Array.isArray(dbMajor.facilities)
                ? dbMajor.facilities
                : typeof dbMajor.facilities === "string"
                ? dbMajor.facilities.split(",").map((s: string) => s.trim())
                : [],
              logo: dbMajor.logo || "",
          ...
          const cachedMajors =
            (slug
              ? localStorage.getItem(`ppdb_majors_config_${slug}`)
              : null) || localStorage.getItem("ppdb_majors_config");
          if (cachedMajors) {
            try {
      ...
      if (activeConfig.ppdb_gelombang_config) {
        ...
        if (g && typeof g === "object") {
          ...
          const gObj = g as any;
          if (
            gObj.gelombang1?.start ||
            gObj.gelombang1?.end ||
            gObj.gelombang2?.start ||
            gObj.gelombang2?.end
          ) {
            setGelombangConfig({
              gelombang1: {
                start:
                  gObj.gelombang1?.start ||
                  DEFAULT_GELOMBANG_UI.gelombang1.start,
                end:
                  gObj.gelombang1?.end || DEFAULT_GELOMBANG_UI.gelombang1.end,
              },
              gelombang2: {
                start:
                  gObj.gelombang2?.start ||
                  DEFAULT_GELOMBANG_UI.gelombang2.start,
                end:
                  gObj.gelombang2?.end || DEFAULT_GELOMBANG_UI.gelombang2.end,
              },
            });
          } else {
      ...
      if (activeConfig.ppdb_landing_active !== undefined) {
        setIsLandingPageActive(
          activeConfig.ppdb_landing_active === true ||
            activeConfig.ppdb_landing_active === "true",
        );
      }
      if (
        activeConfig.ppdb_fields_config &&
        typeof activeConfig.ppdb_fields_config === "object"
      ) {
        setFieldsConfigUI((prev) => ({
          ...prev,
          ...activeConfig.ppdb_fields_config,
        }));
      }

      const initialSnap = buildKelolaUISnapshot({
        ppdb_landing_active:
          activeConfig.ppdb_landing_active !== undefined
            ? activeConfig.ppdb_landing_active === true ||
              activeConfig.ppdb_landing_active === "true"
            : true,
        ppdb_hero_title: activeConfig.ppdb_hero_title || "",
        ppdb_hero_title_sub:
          activeConfig.ppdb_hero_title_sub ||
          (!draft ? `Portal PPDB ${ppdbTitle || "Online"}` : ""),
        ...
        ppdb_address:
          activeConfig.ppdb_address || activeConfig.ppdb_alamat || "",
        ppdb_map_title: activeConfig.ppdb_map_title || "",
        ppdb_map_url:
          activeConfig.ppdb_map_url || activeConfig.ppdb_maps_embed || "",
        ...
        ppdb_alur_config: Array.isArray(activeConfig.ppdb_alur_config)
          ? activeConfig.ppdb_alur_config
          : !isDemo
          ? []
          : DEFAULT_ALUR,
        ppdb_majors_config: isDemo
          ? DEFAULT_MAJORS
          : Array.isArray(activeConfig.ppdb_majors_config)
          ? activeConfig.ppdb_majors_config
          : [],
        ppdb_faq_config: Array.isArray(activeConfig.ppdb_faq_config)
          ? activeConfig.ppdb_faq_config
          : isDemo
          ? DEFAULT_FAQ
          : [],
        ...
        ppdb_partners_config: Array.isArray(activeConfig.ppdb_partners_config)
          ? activeConfig.ppdb_partners_config
          : isDemo
          ? DEFAULT_PARTNERS
          : [],
        ppdb_gelombang_config:
          activeConfig.ppdb_gelombang_config ||
          (isDemo
            ? {
                gelombang1: { start: "2026-01-01", end: "2026-04-30" },
                gelombang2: { start: "2026-05-01", end: "2026-07-15" },
  ...
  }, [adminToken, draftKey, isDemo, ppdbTitle, showToastMsg, slug]);

  // Sub-hook: Revisions & Audit Log
  const { revisions, fetchRevisions, handleRestore, formatDate } =
    useKelolaUIRevisions({
      slug,
      adminToken,
      draftKey,
      showToastMsg,
  ...
  useEffect(() => {
    if (ppdbTitle && ppdbTitle !== "PPDB SMK TB") {
      requestAnimationFrame(() => {
        setSchoolTitle((prev) =>
          prev === "Portal PPDB" || prev.startsWith("PPDB PPDB")
            ? `PPDB ${ppdbTitle}`
            : prev,
        );
        setHeroTitleSub((prev) =>
          prev === "Portal PPDB Online" || prev.startsWith("Portal PPDB PPDB")
            ? `Portal PPDB ${ppdbTitle}`
            : prev,
        );
      });
    }
  ...
  const handleSaveAll = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      if (editingMajor) {
        if (isNewMajor) {
          const exists = finalMajors.some(
            (m) => m.code.toUpperCase() === editingMajor.code.toUpperCase(),
          );
          if (!exists) {
      ...
      finalMajors = finalMajors.map((major) => {
        if (major.gallery && Array.isArray(major.gallery)) {
          return {
            ...major,
            gallery: major.gallery.filter(
              (g: { url?: string }) =>
                g && typeof g.url === "string" && g.url.trim().length > 0,
            ),
          };
        }
        return major;
      ...
      if (json.success) {
        ...
        try {
          ...
          if (slug) {
            localStorage.setItem(
              `ppdb_majors_config_${slug}`,
              JSON.stringify(savedData.ppdb_majors_config || finalMajors),
            );
            localStorage.setItem(
              `ppdb_majors_config`,
              JSON.stringify(savedData.ppdb_majors_config || finalMajors),
            ...
            localStorage.setItem(
              `ppdb_alur_config_${slug}`,
              JSON.stringify(savedData.ppdb_alur_config || alurList),
            );
            localStorage.setItem(
              `ppdb_faq_config_${slug}`,
              JSON.stringify(savedData.ppdb_faq_config || faqList),
            );
            localStorage.setItem(
              `ppdb_partners_config_${slug}`,
              JSON.stringify(savedData.ppdb_partners_config || partnersList),
            );
            localStorage.setItem(
              `ppdb_bank_config_${slug}`,
              JSON.stringify(savedData.ppdb_bank_config || bankConfigList),
            );
            localStorage.setItem(
              `ppdb_fields_config_${slug}`,
              JSON.stringify(fieldsConfigUI),
            );
            localStorage.setItem(
              `ppdb_gelombang_config_${slug}`,
              JSON.stringify(gelombangConfig),
            );
            localStorage.setItem(
              `cation_landing_cache_${slug}`,
        ...
        if (
          savedData.ppdb_majors_config &&
          Array.isArray(savedData.ppdb_majors_config)
        ) {
        ...
        if (
          savedData.ppdb_partners_config &&
          Array.isArray(savedData.ppdb_partners_config)
        ) {
          setPartnersList(savedData.ppdb_partners_config);
        }
        if (
          savedData.ppdb_alur_config &&
          Array.isArray(savedData.ppdb_alur_config)
        ) {
          setAlurList(savedData.ppdb_alur_config);
        }
        if (
          savedData.ppdb_faq_config &&
          Array.isArray(savedData.ppdb_faq_config)
        ) {
          setFaqList(savedData.ppdb_faq_config);
        }
        if (
          savedData.ppdb_bank_config &&
          Array.isArray(savedData.ppdb_bank_config)
        ) {
          setBankConfigList(savedData.ppdb_bank_config);
        }

        const updatedSnap = buildKelolaUISnapshot({
          ppdb_landing_active:
            savedData.ppdb_landing_active !== undefined
              ? savedData.ppdb_landing_active === true ||
                savedData.ppdb_landing_active === "true"
              : true,
          ppdb_hero_title: savedData.ppdb_hero_title || heroTitle || "",
          ppdb_hero_title_sub:
            savedData.ppdb_hero_title_sub || heroTitleSub || "",
          ppdb_hero_subtitle:
            savedData.ppdb_hero_subtitle || heroSubtitle || "",
          ppdb_hero_bg_image:
            savedData.ppdb_hero_bg_image || heroBgImage || "",
          ...
          ppdb_school_period:
            savedData.ppdb_school_period || schoolPeriod || "",
          ppdb_wa_group_url:
            savedData.ppdb_wa_group_url || waGroupUrl || "",
          ppdb_wa_admin: formatPhoneNumber(
            savedData.ppdb_wa_admin || waAdmin || "",
          ),
          ppdb_form_guideline:
            savedData.ppdb_form_guideline || formGuideline || "",
          ...
          ppdb_alur_config: Array.isArray(savedData.ppdb_alur_config)
            ? savedData.ppdb_alur_config
            : alurList,
          ppdb_majors_config: Array.isArray(savedData.ppdb_majors_config)
            ? savedData.ppdb_majors_config
            : finalMajors,
          ppdb_faq_config: Array.isArray(savedData.ppdb_faq_config)
            ? savedData.ppdb_faq_config
            : faqList,
          ppdb_faq_title: savedData.ppdb_faq_title || faqTitle || "",
          ppdb_faq_subtitle:
            savedData.ppdb_faq_subtitle || faqSubtitle || "",
          ppdb_partners_config: Array.isArray(savedData.ppdb_partners_config)
            ? savedData.ppdb_partners_config
            : partnersList,
          ppdb_gelombang_config:
            savedData.ppdb_gelombang_config || gelombangConfig,
          ppdb_fields_config:
            savedData.ppdb_fields_config &&
            typeof savedData.ppdb_fields_config === "object"
              ? {
                  ...DEFAULT_FIELDS_CONFIG_UI,
                  ...savedData.ppdb_fields_config,
                }
  ...
  const handleToggleLandingPageStatus = async () => {
    ...
    const statusText = nextStatus
      ? "DIBUKA (PUBLIK)"
      : "DITUTUP (DRAFT / MAINTENANCE)";

    if (nextStatus === true && slug !== "demo" && slug !== "smktarunabhakti") {
      try {
        const subRes = await fetch(
          `/api/saas/school-by-slug/${slug}?t=${Date.now()}`,
        );
        const subData = await subRes.json();
        const schoolObj = subData?.data;
        const isPaid =
          schoolObj?.plan_type &&
          schoolObj.plan_type !== "FREE" &&
          schoolObj.plan_type !== "free_trial" &&
          schoolObj.plan_type !== "TRIAL";
        const isVerified =
          schoolObj?.status === "FULL_VERIFIED" ||
          schoolObj?.status === "VERIFIED";
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        try {
          ...
          const token =
            adminToken || localStorage.getItem("ppdb_admin_token");
