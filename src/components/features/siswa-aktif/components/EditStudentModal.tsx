...
interface EditStudentModalProps {
  editApplicant: ActiveStudent | null;
  editForm: Partial<ActiveStudent>;
  setEditForm: React.Dispatch<React.SetStateAction<Partial<ActiveStudent>>>;
  isSaving: boolean;
  onClose: () => void;
  onSave: () => void;
}
...
export const EditStudentModal: React.FC<EditStudentModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-60 flex items-center justify-center p-4 lg:p-8 bg-black/70 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white dark:bg-[#0f172a] border border-slate-200 dark:border-slate-800/80 rounded-3xl w-full max-w-4xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden animate-in zoom-in-95 transition-all text-left">
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto p-6 md:p-8 space-y-6 bg-slate-50 dark:bg-slate-950/20">
          {sections.map((section) => (
            <div key={section.section} className="bg-white dark:bg-[#0f172a] border border-slate-200 dark:border-slate-800/60 rounded-3xl p-6 md:p-8 shadow-xs">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                {section.fields.map((f) => (
                  <div key={f.key} className="group">
                    <label className="block text-[10px] font-black uppercase tracking-widest text-slate-455 dark:text-slate-400 mb-2 group-focus-within:text-blue-500 dark:group-focus-within:text-blue-400 transition-colors">
                      {f.label}
                    </label>
                    {f.type === "select" ? (
                      <Select
