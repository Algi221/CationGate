...
async function saveBase64File(base64Str: string, prefix: string): Promise<string> {
...
configRouter.post('/', adminAuth, async (c) => {
  try {
    ...
    if (key === 'ppdb_majors_config') {
      ...
      processedValue = await saveBase64File(value, 'school_logo');
    }
...
configRouter.post('/save-all', adminAuth, async (c) => {
  try {
    ...
    if (processedConfigs.ppdb_logo_url) {
      processedConfigs.ppdb_logo_url = await saveBase64File(processedConfigs.ppdb_logo_url, 'school_logo');
    }
